import { useGetCharacter, useGetFollowers, useGetFollowings, useGetNotes, useGetAchievement, useGetSync } from "../queries/character"
    ...
    <div className="relative flex flex-col items-center justify-center min-h-screen py-20">
      ...
      >
        ...
        <div className="flex relative">
          <div className="absolute right-0 top-0 bg-blue-400 text-white px-12 py-1 rounded-2xl cursor-pointer">Follow</div>
          <div className="w-32 text-center mr-4 flex flex-col items-center justify-between">
            {character.data?.metadata?.content?.avatars && <img className="rounded-full inline-block" alt={handle} src={character.data?.metadata?.content?.avatars?.[0]?.replace("ipfs://", IPFS_GATEWAY) || ""} width={80} height={80} />}
            ...
            <div className="mt-2">
              <div className="text-xs">
                ...
                <Link target="_blank" href={`https://scan.crossbell.io/address/${character.data?.owner}`}>
                  {character.data?.owner.slice(0, 5)}...{character.data?.owner.slice(-4)}
                </Link>
              </div>
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-medium text-2xl"><span>{character.data?.metadata?.content?.name}</span><span className="text-base ml-2">@{handle}</span></p>
            <p className="truncate text-sm mt-1">{character.data?.metadata?.content?.bio}</p>
            <div className="space-x-5 mt-2">
              <Link href={`https://crossbell.io/@${handle}/followers`} target="_blank">{followers.data?.count} Followers</Link>
              <Link href={`https://crossbell.io/@${handle}/following`} target="_blank">{followings.data?.count} Following</Link>
              <span>{notes.data?.count} Notes</span>
            </div>
            <div className="text-gray-500 mt-2 text-sm">
              <Link target="_blank" href={`https://scan.crossbell.io/tx/${character.data?.transactionHash}`}>
                Joined{" "}
                {dayjs
                  .duration(
            ...
            <div className="mt-3 text-[0px]">
              {achievement.data?.list[0]?.groups?.map((group) => {
                ...
                return <span className="mr-4 inline-flex" key={achievement.tokenId}>
                  <img className="w-10 h-10 inline-block mr-2" src={achievement.media.replace("ipfs://", IPFS_GATEWAY)} />
                  <span className="inline-flex flex-col justify-around">
                    <span className="capitalize text-sm">{group.info.title}</span>
                    <span className="text-xs">{achievement.attributes.find((attribute) => attribute.trait_type === "tier")?.value}</span>
                  </span>
                </span>
              })}
            </div>
      ...
      <div className="w-[800px] text-sm mt-8 relative rounded-3xl text-gray-700 border-2 border-gray-100 overflow-hidden backdrop-blur-md py-6 px-8">
        <div className="font-medium text-base mb-4">
          <span className="align-middle">I am owning these social contents on Crossbell</span>
          <Link href="https://crossbell.io/sync" target="_blank" className="align-middle ml-2 inline-flex justify-center">
        ...
        <div className="mb-4">
          {sourceList.xlog && <Link className="mr-6 inline-flex" target="_blank" href={syncMap["xlog"].url.replace("{username}", handle)}>
            <img className="rounded-full w-9 h-9 inline-block mr-2" src={syncMap["xlog"].icon} />
            <span className="inline-flex flex-col justify-around">
          ...
          {sync.data?.data?.result?.map((item: any) => {
            return <Link className="mr-6 inline-flex" key={item.platform} target="_blank" href={syncMap[item.platform].url.replace("{username}", item.username)}>
              <img className="rounded-full w-9 h-9 inline-block mr-2" src={syncMap[item.platform].icon} />
              <span className="inline-flex flex-col justify-around">
      ...
      <div className="w-[800px] text-sm mt-8 relative rounded-3xl text-gray-700 border-2 border-gray-100 overflow-hidden backdrop-blur-md py-6 px-8">
        ...
        <div className="font-medium text-base relative">My on-chain social contents</div>
        {notes.data?.list.map((note) => {
          return <div key={note.noteId}
            className="mx-auto relative py-6 space-y-2 overflow-hidden"
          >
            <Link href={note.metadata?.content?.sources?.includes("xlog") && note.metadata?.content?.external_urls?.[0] ? note.metadata?.content?.external_urls?.[0] : `https://crossbell.io/notes/${note.characterId}-${note.noteId}`} target="_blank">
              <div className="text-gray-400 relative">{dayjs
                .duration(
            ...
            <div className="flex justify-between items-center">
              <div className="text-xs relative">
                {note.metadata?.content?.external_urls?.[0] && <Link href={note.metadata?.content?.external_urls?.[0]} target="_blank">
                  {note.metadata?.content?.sources?.map((source) => <span className="bg-gray-300 rounded-3xl px-2 inline-block mt-1 mr-1" key={source}>{source}</span>)}
                </Link>}
                {!note.metadata?.content?.external_urls?.[0] && note.metadata?.content?.sources?.map((source) => <span className="bg-gray-300 rounded-3xl px-2 inline-block mt-1 mr-1" key={source}>{source}</span>)}
              </div>
              <div className="mr-1 text-gray-400 relative">
                <Link target="_blank" href={`https://scan.crossbell.io/tx/${note.updatedTransactionHash}`}>
                  #{note.noteId} {note.updatedTransactionHash.slice(0, 5)}...{note.updatedTransactionHash.slice(-4)}
                </Link>
              </div>
            </div>
          </div>
