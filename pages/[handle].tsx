import { useGetCharacter, useGetFollowers, useGetFollowings, useGetNotes, useGetAchievement } from "../queries/character"
...
export default function HandlePage() {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen py-20">
      <div className="fixed left-1/2 -translate-x-1/2 top-28">
      ...
      <Tilt
        className="parallax-effect w-[800px] mx-auto relative p-8 rounded-3xl text-gray-600 border-2 border-gray-100 overflow-hidden backdrop-blur-md"
        glareEnable={true}
        glareMaxOpacity={0.2}
        glareColor="#fff"
        glarePosition="all"
      ...
      >
        ...
        <div className="inner-element flex relative">
          <div className="w-32 text-center mr-4 flex flex-col items-center justify-between">
            ...
            <div className="mt-2">
              ...
              <div className="mt-2 text-xs">
                Blockchain Record<br/>
                <Link target="_blank" href={`https://scan.crossbell.io/tx/${character.data?.transactionHash}`}>
                  {character.data?.transactionHash.slice(0, 5)}...{character.data?.transactionHash.slice(-4)}
                </Link>
              </div>
            </div>
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="font-medium space-x-4 mt-2">
              <span>{followers.data?.count} Followers</span>
              <span>{followings.data?.count} Following</span>
            ...
            <div className="mt-3 mb-3">
              {achievement.data?.list[0]?.groups?.map((group) => {
                ...
                return <span className="mr-4 inline-flex" key={achievement.tokenId}>
                  <img className="w-9 h-9 inline-block mr-2" src={achievement.media.replace("ipfs://", IPFS_GATEWAY)} />
            ...
            <div className="text-xs mt-2 leading-snug">{Object.keys(sourceList).sort((a, b) => sourceList[b] - sourceList[a]).map((source) => {
              return <span className="bg-gray-400 rounded-3xl px-2 inline-block mt-1 mr-1" key={source}>{source + " " + sourceList[source]}</span>
            })}</div>
          </div>
        </div>
      </Tilt>
      <div className="w-[800px] text-sm mt-8 relative rounded-3xl text-gray-700 border-2 border-gray-100 overflow-hidden backdrop-blur-md">
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-gray-50 to-gray-50 opacity-70"></div>
        {notes.data?.list.map((note) => {
          return <div key={note.noteId}
            className="mx-auto relative p-8 space-y-2 overflow-hidden"
          >
            ...
            <div className="flex justify-between items-center">
              <div className="text-xs relative">
                {note.metadata?.content?.sources?.map((source) => <span className="bg-gray-400 rounded-3xl px-2 inline-block mt-1 mr-1" key={source}>{source}</span>)}
              </div>
